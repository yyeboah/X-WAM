...
def sample_beta(shape, generator, alpha=1.0, beta=1.0):
    alpha_tensor = torch.full(shape, alpha, dtype=torch.float32)
    beta_tensor = torch.full(shape, beta, dtype=torch.float32)
    X = torch._standard_gamma(alpha_tensor, generator=generator)
    Y = torch._standard_gamma(beta_tensor, generator=generator)
    return X / (X + Y).clamp(min=1e-6)
